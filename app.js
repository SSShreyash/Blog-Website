...
const blogSchema = new mongoose.Schema({head: String, body: String});
...
app.post("/compose", function(req, res){
    const newPost = {head:req.body.newContentHead, body:req.body.newContent}
  ...
  Post.find().then(function (posts){
    posts.forEach(function(post){
        if(_.lowerCase(post._id)===postId){
            res.render("post", {PostHeading: post.head, PostContent: post.body});
        }
    });
  }).catch(function (err){console.log(err)});
});
